			...
			<div style="overflow-x:auto;">
				<table <?php tabel(100, '%', 1, 'left');  ?>>
					<tr style="background: black; color:White">
						<th>No</th>
						<th align="center" class="th_border cell"><?php $nama_produk = str_replace('_', ' ', $nama_produk);
																											echo ucwords($nama_produk); ?>, Catatan</th>
					...
					<tbody>
						...
						?>
							<tr class="event2">
								<td align="left" width="50"><?php $no = (($no + 1));
																						echo $no;  ?></td>
								<td align="left">
								...
								<td align="left">
									<font color="green"><b><?php echo rupiah($data["$harga"]); ?></b></font>
								</td>
								<td align="left">
									<form action="" method="get">
										...
										<?php $jumlah_keseluruhan = $jumlah_keseluruhan +  ($data["$jumlah"]);
										echo ($data["$jumlah"]); ?>
								...
								<td align="left">
									<font color="green"><b><?php
																					...
																					echo rupiah(($data["$harga"]) * ($data["$jumlah"])); ?></b></font>
								</td>

								<td class="th_border cell" align="left" width="200">
									<center>
										<b><?php $s = $data['status'];   ?> <?php $st = str_replace('_', ' ', $s);
																												echo ucwords($st); ?></b>
									</center>
									<br>
									<?php
									...
									?>
										...
										?>
											...
											<center>
												<img onerror="this.src='home/data/image/error/error.png'" src="admin/upload/
	<?php
							...
							<table border="0">
								<tr>
									<td>
										...
										<?php } elseif ($s == "booking") {
										?>
											<a href="<?php index(); ?>?p=transaksi&action=upload_bukti&berapa=<?php echo ($total_keseluruhan); ?>&proses=<?php echo $kodenya; ?>">
												<?php btn_edit('Upload Bukti Pembayaran'); ?></a>

											<?php } else {
											$kategori_bayar = baca_database('', 'kategori_pembayaran', "select * from data_kategori_pembayaran where kode_transaksi_pembayaran='$kodenya'");

											if ($kategori_bayar == "bpjs") { 	?>

												<a href="<?php index(); ?>?p=transaksi&action=upload_bukti&berapa=<?php echo ($total_keseluruhan); ?>&proses=<?php echo $kodenya; ?>">
													<?php btn_edit('Upload Kartu BPJS untuk Pembayaran'); ?></a>
